# Website

Official documentation and resources for tsdown.

## Official Documentation

| Resource | URL |
|----------|-----|
| **Homepage** | https://tsdown.dev/ |
| **Documentation** | https://tsdown.dev/guide/ |
| **GitHub** | https://github.com/rolldown/tsdown |
| **npm** | https://www.npmjs.com/package/tsdown |

## Core Docs

| Page | URL |
|------|-----|
| **Getting Started** | https://tsdown.dev/guide/getting-started |
| **Introduction** | https://tsdown.dev/guide/ |
| **Configuration** | https://tsdown.dev/guide/configuration |

## Advanced

| Page | URL |
|------|-----|
| **Plugins** | https://tsdown.dev/advanced/plugins |
| **DTS Options** | https://tsdown.dev/options/dts |

## Related Projects

| Project | URL |
|---------|-----|
| **Rolldown** | https://rolldown-vite.github.io/rolldown/ |
| **Rollup** | https://rollupjs.org/ |
| **unplugin** | https://github.com/unjs/unplugin |
| **oxc** | https://oxc-project.github.io/ |

## Community Resources

| Resource | URL |
|----------|-----|
| **GitHub Issues** | https://github.com/rolldown/tsdown/issues |
| **GitHub Discussions** | https://github.com/rolldown/tsdown/discussions |
| **Discord** | https://discord.gg/rolldown |

## StackBlitz

Try tsdown online: https://stackblitz.com/

## Related Packages

| Package | URL |
|---------|-----|
| **typescript** | https://www.typescriptlang.org/ |
| **rolldown-plugin-dts** | https://github.com/rolldown/rolldown-plugin-dts |