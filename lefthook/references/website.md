# Website and Resources

## Official Resources

| Resource | URL |
|----------|-----|
| Website | https://lefthook.dev |
| GitHub | https://github.com/evilmartians/lefthook |
| Documentation | https://github.com/evilmartians/lefthook#readme |
| npm | https://www.npmjs.com/package/lefthook |

## Installation

| Method | Command |
|--------|---------|
| npm | `npm install --save-dev lefthook` |
| Go | `go install github.com/evilmartians/lefthook@latest` |
| Homebrew | `brew install lefthook` |
| GitHub Releases | Download binary from releases page |

## Community

| Resource | URL |
|----------|-----|
| Discussions | https://github.com/evilmartians/lefthook/discussions |
| Issues | https://github.com/evilmartians/lefthook/issues |
| Evil Martians Blog | https://evilmartians.com/chronicles |

## Related Tools

| Tool | Description |
|------|-------------|
| Husky | Git hooks manager (Node.js) |
| pre-commit | Git hooks manager (Python) |
| lint-staged | Run linters on staged files |
| commitlint | Lint commit messages |

## Version

Current stable version: See [GitHub releases](https://github.com/evilmartians/lefthook/releases)